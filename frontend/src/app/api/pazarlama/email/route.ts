...
export async function POST(req: NextRequest) {
  ...
  const { subject, body, discount_code } = await req.json();
  ...
  const unique = [...new Set(emails.map((e) => e.email).filter(Boolean))] as string[];
  ...
  const htmlBody = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
      ...
      <p style="color:#9ca3af;font-size:12px;text-align:center">
        Bu e-posta ${shopName} tarafından gönderilmiştir.
        SatıcıPilot altyapısıyla iletilmektedir.
      </p>
    </div>
  `;
